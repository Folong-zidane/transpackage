...
const ProcessExistingPackage: React.FC<ProcessExistingPackageProps> = ({ onBackToSelection }) => {
  ...
  const handleFinalizeDeposit = () => {
    setShowSuccessMessage(`Colis ${packageInfo?.trackingNumber} enregistré comme déposé !`);
    setTimeout(() => {
      setPackageInfo(null); setSearchInput(''); setPaymentValidated(false);
      setAmountReceived(''); setShowSuccessMessage(null);
      // onBackToSelection(); // Optionnel
    }, 2500);
  };
